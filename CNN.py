...
class NeuralNet(nn.Module):
    def __init__(self, features, filename):
        ...
        self.fc1 = nn.Linear(features, 15, bias = False)
        self.fc2 = nn.Linear(15, 10, bias = False)
        self.fc3 = nn.Linear(10, 1, bias = False)
#        self.fc4 = nn.Linear(5, 1, bias = False)
        self.file = filename
    def initialize(self):
        nn.init.xavier_uniform(self.linear.weight.data)
        self.linear.bias.data.zero_()
    ...
    def forward(self,x):
        ...
        CASI.write_progress_to_file(self.file, 'NeuralNet-1st-lyr', str(out.size()))
        
        out = F.relu(self.fc2(out))
        CASI.write_progress_to_file(self.file, 'NeuralNet-2nd-lyr', str(out.size()))

        out = F.relu(self.fc3(out))
        CASI.write_progress_to_file(self.file, 'NeuralNet-3rd-lyr', str(out.size()))
